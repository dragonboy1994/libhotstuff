...
//TODO: change it to a CLASS & clear all vectors before using them
std::vector<OrderedList> aequitas (std::vector<OrderedList> proposed_orderlist, float g = 3.0 / 4.0)
{
    int n_replica = proposed_orderlist.size();
    if(n_replica == 0) 
    ...
    int distinct_cmd = 0;
    std::vector<uint256_t> cmd_content; cmd_content.push_back("0");
    std::map<uint256_t, int> map_cmd;

    //TODO: embed into class...
    ...
    vector<OrderedList> final_ordered_vector;
    for (int i = 1; i <= scc; i++)
    {
    ...
    While(!que.empty())
    {
        ...
        While(!que.empty())
        {
            ...
            {
                cmds.push_back(cmd_content[scc_have[i]]);
